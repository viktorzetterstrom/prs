...
func (d itemDelegate) Render(w io.Writer, m list.Model, index int, listItem list.Item) {
	...
	// Budget the title so the right-hand emoji is always visible. The selected
	// and unselected rows both consume 4 cells before content (4 spaces of
	// padding, or 2 spaces + "➤ "), and 3 single-space separators sit between
	// the four content segments. Leave 1 trailing cell as breathing room so
	// the emoji never sits at the very last column — some terminals
	// half-clip a 2-cell emoji that lands exactly at the edge.
	title := pr.Title
